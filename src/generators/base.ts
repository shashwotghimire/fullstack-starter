...
function serverPackageJson(options: ScaffoldOptions): string {
  const deps = [
    '"cors": "^2.8.5"',
    '"express": "^5.0.1"',
    options.orm === "prisma" ? '"@prisma/client": "^6.0.1"' : "",
    options.orm === "drizzle" ? '"drizzle-orm": "^0.36.4"' : "",
    options.orm === "drizzle" ? '"postgres": "^3.4.5"' : "",
    options.orm === "sequelize" ? '"pg": "^8.21.0"' : "",
    options.orm === "sequelize" ? '"pg-hstore": "^2.3.4"' : "",
    options.orm === "sequelize" ? '"sequelize": "^6.37.8"' : "",
  ...
  const devDeps = [
    ...
    options.orm === "prisma" ? '"prisma": "^6.0.1"' : "",
    options.orm === "drizzle" ? '"drizzle-kit": "^0.27.2"' : "",
    options.orm === "sequelize" ? '"sequelize-cli": "^6.6.3"' : "",
  ...
  const dbScripts =
    options.orm === "prisma"
      ...
      : options.orm === "drizzle"
        ...
        : {
            "db:generate": "echo \"Sequelize models are generated in source\"",
            "db:migrate":
              "sequelize-cli db:migrate --config ../config/config.js --migrations-path ../sequelize/migrations",
          };
  ...
  const ormScripts =
    options.orm === "prisma"
      ...
      : options.orm === "sequelize"
        ? "- `bun run db:migrate`: run pending Sequelize migrations\n"
      : "";
...
- Tailwind CSS v4 is configured through \`@tailwindcss/vite\` and \`client/src/index.css\`
- shadcn/ui is configured with \`client/components.json\`
- Starter UI components live in \`client/src/components/ui\`
...
The server starts even when PostgreSQL is unavailable, so \`/api/health\` can verify frontend/API connectivity before the database is running.
`;
}
...
app.listen(port, () => {
  ...
  console.info("Database: lazy Prisma connection");
});
`;
  }
...
void connectDb().then(
  ...
  (error: unknown) => {
    ...
    console.warn(\`Database: unavailable (\${message})\`);
  },
);
...
app.listen(port, () => {
  console.info(\`Server listening on port \${port}\`);
  console.info(\`Environment: \${getNodeEnv()}\`);
  console.info(\`Database startup status: \${dbStatus}\`);
});
`;
}
...
`;
  ...
  const files: FileEntry[] = [
    ...
    { path: "client/components.json", contents: readTemplate("base/client/components.json") },
    { path: "client/src/index.css", contents: readTemplate("base/client/src/index.css") },
    { path: "client/src/lib/utils.ts", contents: readTemplate("base/client/src/lib/utils.ts") },
    { path: "client/src/components/ui/badge.tsx", contents: readTemplate("base/client/src/components/ui/badge.tsx") },
    { path: "client/src/components/ui/button.tsx", contents: readTemplate("base/client/src/components/ui/button.tsx") },
    { path: "client/src/components/ui/card.tsx", contents: readTemplate("base/client/src/components/ui/card.tsx") },
    {
    ...
    { path: "server/src/constants/index.ts", contents: readTemplate("base/server/src/constants/index.ts") },
    {
      ...
      contents: `export const routes = { home: "/", health: "/health" } as const;
export const authTokenStorageKey = "create-pern-app-token";
`,
    },
